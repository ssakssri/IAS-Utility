...
const schema = z.object({ userIds: z.array(z.string()).min(1).max(100) });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const results = await throttledBulkProcess(
      ...
      async (userId) => {
        ...
        const logId = await auditLogService.create({
          ...
          action: 'BULK_PASSWORD_RESET_EMAIL',
          targetUserId: userId,
          targetUserName,
        });

        try {
          await service.sendPasswordResetEmail(userId);
    ...
    return NextResponse.json({ total: body.userIds.length, success, failed: body.userIds.length - success, results: mapped });
  } catch (e) {
